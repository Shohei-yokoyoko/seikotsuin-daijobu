...
const categories = [
  ["01","健康保険・療養費","保険が使える条件と、患者が確認したいこと","/categories/health-insurance"],
...
<section className="section category-section" id="categories"><div className="section-head"><div><span className="kicker">EXPLORE</span><h2>知りたいことから<br/>確かめる。</h2></div><p>記事数が増えても、必要な情報へ迷わず進めるための分類です。</p></div><div className="category-grid">{categories.map(([num,title,desc,href])=>href?<Link href={href} className="category-card" key={title}><span>{num}</span><h3>{title}</h3><p>{desc}</p><i>↗</i></Link>:<div className="category-card unavailable" key={title}><span>{num}</span><h3>{title}</h3><p>{desc}</p><em>記事準備中</em></div>)}</div></section>
...
<section className="sources section" id="sources"><span className="kicker">SOURCES MATTER</span><h2>出典を、<br/>読者が追える形に。</h2><p>各記事の末尾に「この記事で確認した主な資料」を設け、資料名・発行元・年・資料の種類を整理します。単なるURLの羅列にはしません。</p><div className="source-tags"><span>法令</span><span>厚生労働省</span><span>自治体・保険者</span><span>診療ガイドライン</span><span>論文・研究</span></div></section>
</main><Footer/></>}
